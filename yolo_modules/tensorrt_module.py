...
def do_inference_wrapper(engine_wrapper, net_img):
    net_img = np.expand_dims(net_img.transpose(2, 0, 1), axis=0)  # (1, 3, 320, 512)
    engine_wrapper[1][0].host = np.ascontiguousarray(net_img, dtype=np.float32) / 255.
    trt_output = do_inference(engine_wrapper[0],
                              inputs=engine_wrapper[1],
                              outputs=engine_wrapper[2],
                              bindings=engine_wrapper[3],
                              stream=engine_wrapper[4])
    return trt_output
